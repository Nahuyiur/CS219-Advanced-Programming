        ...
        switch (ch) {
            ...
            case '$':
                if (!lines[cursor_y].empty()) {
                    cursor_x = lines[cursor_y].length() - 1; // [MODIFIED] 设置到最后一个字符
                } else {
                    cursor_x = 0; // 空行光标设置为0
                }
                adjust_window(); // [MODIFIED] 更新窗口以反映光标位置
                break;
            case 'g':
                if (getch() == 'g') {
                    cursor_y = 0;
    ...
    void insert_mode(int ch) {
        switch (ch) {
            ...
            case KEY_BACKSPACE:
                ...
                break;
            default:
                // [ADDED] 检查光标位置是否超出当前行长度
                if (cursor_x > lines[cursor_y].length()) {
                    lines[cursor_y].append(cursor_x - lines[cursor_y].length(), ' '); // [ADDED] 用空格填充
                }
